...
StringReplace,readme,readme,[RJ_PROJ],%RJPRJCT%
...
if (ServerPush = 1)
	{
		...
		fileappend,pushd "%GITROOT%"`n,%home%\gpush.cmd
		fileappend,git config --global user.name %GITUSER%`n,%home%\gpush.cmd
		fileappend,git config --global user.email %GITMAIL%`n,%home%\gpush.cmd
		fileappend,git -C "%GITROOT%\%GITUSER%.github.io" init`n,%home%\gpush.cmd
		fileappend,git -C "%GITD%" init`n,%home%\gpush.cmd
		fileappend,git remote add %RJPRJCT% %GITWEB%/%GITUSER%/%RJPRJCT%`n,%home%\gpush.cmd
		fileappend,git remote add %GITUSER%.github.io %GITWEB%/%GITUSER%/%GITUSER%.github.io`n,%home%\gpush.cmd
		fileappend,gh auth login --scopes repo`,delete_repo`n,%home%\gpush.cmd
		FileAppend,gh repo create %RJPRJCT% --public --source="%GITD%"`n,%home%\gpush.cmd
		FileAppend,gh repo create %GITUSER%.github.io --public --source="%GITROOT%\%GITUSER%.github.io"`n,%home%\gpush.cmd
		FileAppend, pushd "%GITD%"`n,%DEPL%\gpush.cmd
		FileAppend,git commit -m `%1`%`n,%DEPL%\gpush.cmd
		FileAppend,git push -f --all %RJPRRJCT%`n,%DEPL%\gpush.cmd
		FileAppend,pushd "%GITROOT%\%GITUSER%.github.io"`n,%DEPL%\gpush.bat
		FileAppend,git -C "%GITROOT%\%GITUSER%.github.io" init`n,%DEPL%\gpush.bat
		FileAppend,git add %RJPRJCT%`n,%DEPL%\gpush.bat
		FileAppend,git remote add %GITUSER%.github.io %GITWEB%/%GITUSER%/%GITUSER%.github.io`n,%DEPL%\gpush.bat
		FileAppend,git commit -m siteupdate`n,%DEPL%\gpush.bat
		FileAppend,git push -f --all %GITUSER%.github.io`n,%DEPL%\gpush.bat
		;`
		SB_SetText(" Uploading to server ")
		if (PortVer = 1)
			{
...
;msgbox,,,nsitst=%nsitst%`n%sOutput%`nstdout=%stdOut%ner=%errorlevel%`ncb=%callBackFuncObj%`npr=%hPipeRead%`npw=%hPipeWrite%
sleep,1500
